...
class BlockManager:
    ...
    def compute_hash(cls, token_ids: list[int], prefix: int = -1):
        ...
        h = xxhash.xxh64()
        if prefix != -1:
            h.update(prefix.to_bytes(4, byteorder="little", signed=False))
    ...
    def allocate(self, seq: Sequence):
        ...
        for i in range(seq.num_blocks):
            ...
            if block_id == -1 or self.blocks[block_id].token_ids != token_ids:
                # Cache miss, need to allocate a new block
                cache_miss = True

            if cache_miss:
                block_id = self.free_block_ids[0]  # Get the first free block ID
